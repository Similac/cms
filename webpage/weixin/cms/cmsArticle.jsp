...
<html>
 <head>
  ...
  $(function () {
      ...
      $('#columnId').combotree({
          url : 'cmsMenuController.do?combotree&siteId=${cmsArticlePage.siteid}&pCode=',
          panelHeight : 200,
          width : 157,
      ...
      $('#siteid').change(function(){
          ...
          $('#columnId').combotree({
              ...
              onLoadSuccess: function (node,data) { //加载完成后,设置选中第一项
                 if($thisval == '${cmsArticlePage.siteid}'){
                     $('#columnId').combotree('setValue','${cmsArticlePage.columnId}');
                 }else{
                     if(data.length>0){
			...
			<div class="appmsg editing">
				<div class="appmsg_content" id="js_appmsg_preview">
						...
						<div id="files" class="files">
							<i class="appmsg_thumb default" id="imageShow">首页新闻图片(660px*280px)</i>
						</div>
			...
			<div class="appmsg_editor" style="margin-top: 0px;">
		 		<div class="inner" style="width: 90%">
					...
					<input id="id" name="id" type="hidden" value="${cmsArticlePage.id }">
                        <c:choose>
                            <c:when test="${cmsArticlePage.viewcount ==null || cmsArticlePage.viewcount ==''}">
                                <input id="viewcount" name="viewcount" type="hidden" value="0">
                            </c:when>
                            <c:otherwise>
                                <input id="viewcount" name="viewcount" type="hidden" value="${cmsArticlePage.viewcount }">
                            </c:otherwise>
                        </c:choose>
                        <input id="createDate" name="createDate" type="hidden" value="${cmsArticlePage.createDate }">
					...
					<table cellpadding="0" cellspacing="1" class="formtable">
						<tr>
							...
							<td class="value">
								<input class="inputxt" id="title" name="title" style="width: 500px" value="${cmsArticlePage.title}" datatype="*" onblur="setimageTitle(this)">
                        ...
                        <tr>
                            ...
                            <td class="value">
                                <select id="siteid" name="siteid" style="width: 250px">
                                    <option value="">请选择</option>
                                    <c:forEach var="sitelist" items="${CmsSiteList}">
                                        <option <c:if test="${sitelist.id==cmsArticlePage.siteid}"> selected </c:if> value="${sitelist.id}">${sitelist.siteName}</option>
                                    </c:forEach>
                                </select>
                                <span class="Validform_checktip"></span>
						...
						<tr>
							...
							<td class="value">
                                <input id="columnId" name="columnId" value="${cmsArticlePage.columnId}">
								<span class="Validform_checktip"></span>
								<label class="Validform_label" style="display: none;">所属栏目</label>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="value">
								<textarea class="inputxt" id="summary" name="summary" style="width: 350px" datatype="*">${cmsArticlePage.summary}</textarea>
								<span class="Validform_checktip"></span>
                        ...
                        <tr >
                            ...
                            <td class="value">
                                <c:if test="${cmsArticlePage.istop==null}">
                                    <t:dictSelect field="istop" id="istop" typeGroupCode="yesorno" hasLabel="false" defaultVal="N"></t:dictSelect>
                                </c:if>
                                <c:if test="${cmsArticlePage.istop!=null}">
                                    <t:dictSelect field="istop" id="istop" typeGroupCode="yesorno" hasLabel="false" defaultVal="${cmsArticlePage.istop}"></t:dictSelect>
                                </c:if>
                        ...
                        <td class="value">
                            <input  name="topdate" type="text" style="width: 150px"
                                   class="Wdate" onClick="WdatePicker()"

                                   value='<fmt:formatDate value='${cmsArticlePage.topdate}' type="date" pattern="yyyy-MM-dd"/>'>
                            <span class="Validform_checktip"></span>
                            <label class="Validform_label" style="display: none;">置顶日期</label>
                        </td>
                        </tr>
                        <tr >
                            <td align="right">
                            ...
                            <td class="value">
                                <c:if test="${cmsArticlePage.iscomment==null}">
                                    <t:dictSelect field="iscomment" id="iscomment" typeGroupCode="yesorno" hasLabel="false" defaultVal="Y" ></t:dictSelect>
                                </c:if>
                                <c:if test="${cmsArticlePage.iscomment!=null}">
                                    <t:dictSelect field="iscomment" id="iscomment" typeGroupCode="yesorno" hasLabel="false" defaultVal="${cmsArticlePage.iscomment}" ></t:dictSelect>
                                </c:if>
                                <span class="Validform_checktip"></span>
						...
						<tr>
                            ...
                            <td >
                                <textarea name="content" id="content">${cmsArticlePage.content}</textarea>
                                <script type="text/javascript">
